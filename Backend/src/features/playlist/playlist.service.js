...
import { PLAYLIST_LIMITS } from "../../core/constants/index.js";
...
class PlaylistService {
    ...
    async createPlaylist(
        ...
        const saved = await playlistRepository.findByIdPopulated(created._id);
        return mapPlaylist(saved, userId);
    }

    async getPlaylist(playlistId, userId) {
        const playlist = await playlistRepository.findByIdPopulated(playlistId);
    ...
    async updatePlaylist(playlistId, userId, body) {
        ...
        const updated = await playlistRepository.updateByIdPopulated(playlistId, updateData);
    ...
    // ─── Collaborators ──────────────────────────────────────
    async addCollaborators(playlist, userId, collaboratorIds) {
        const parsed = parseCollaboratorIds(collaboratorIds);
        if (!parsed?.length)
        ...
        const validUsers = await UserModel.aggregate([
            {
                $match: {
                    _id: {
                        $in: requested.map((id) => new mongoose.Types.ObjectId(id)),
                    },
                },
            },
            { $project: { _id: 1 } },
        ...
        const updated = await playlistRepository.updateWithPipelinePopulated(
            { _id: playlist._id },
            [
                {
                    $set: {
    ...
    async removeCollaborator(playlist, userId, collaboratorUserId) {
        if (!mongoose.Types.ObjectId.isValid(collaboratorUserId)) {
            throw ApiError.badRequest("Invalid user ID");
        }
        const updated = await playlistRepository.updateByIdPopulated(playlist._id, {
    ...
    // ─── Tracks ─────────────────────────────────────────────
    async addTrack(playlist, userId, trackId) {
        ...
        const track = await trackRepository.findById(trackId, { select: "_id" });
        ...
        const updated = await playlistRepository.findByIdPopulated(playlist._id);
        return mapPlaylist(updated, userId);
    }

    async removeTrack(playlist, userId, trackId) {
    ...
    async reorderTracks(playlist, userId, sourceIndex, destinationIndex) {
        ...
        if (from === to) {
            const same = await playlistRepository.findByIdPopulated(playlist._id);
    ...
    // ─── Listings & Search ──────────────────────────────────
    async listMyPlaylists(userId) {
        const playlists = await playlistRepository.findForUser(userId);
        return Promise.all(playlists.map((p) => mapPlaylist(p, userId)));
    }

    async searchUsersForCollaborators(currentUserId, queryString) {
    ...
    async searchTracksForPlaylist(queryString, excludePlaylistId) {
        const tracks = await trackRepository.searchByTitle(queryString);
        let excluded = new Set();
        if (
            excludePlaylistId &&
            mongoose.Types.ObjectId.isValid(excludePlaylistId)
        ) {
            const playlist = await playlistRepository.selectTracks(excludePlaylistId);
            if (playlist) {
