...
const buildPaginated = async ({ basePipeline, query }) => {
    ...
    const [tracks, totalResult] = await Promise.all([
        recommendationRepository.aggregateTracks(dataPipeline),
        recommendationRepository.aggregateTracks(countPipeline),
    ]);
...
class RecommendationService {
    async recommendedForYou(query, userId) {
        let basePipeline = [];
        if (userId) {
    ...
    async episodesYouMightLike(query, userId) {
        let excludeIds = [];
        if (userId) {
            const user = await userRepository.findById(userId, {
                select: "likedTracks",
